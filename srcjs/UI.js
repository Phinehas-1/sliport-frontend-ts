...
const changeTableHeadingSortOrder = (column) => {
...
export const getSortedReportForTotalsTable = (column) => {
    changeTableHeadingSortOrder(column);
    let tableBody = document.getElementById('totals-table-body');
    tableBody === null || tableBody === void 0 ? void 0 : tableBody.remove();
    // sort reports in ascending order from lowest to highest.
    if (column.dataset.order === 'ASC') {
        let reports = Object.values(new Repository(new LocalStorageStore()).getReports());
        reports.sort((a, b) => {
